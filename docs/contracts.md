...
- Déterminisme : ordre = ordre des images du parseur ; chaque visual conserve id, SHA-256 des bytes d'origine, mime déclaré, provenance, part/content ids et `local_ref` ; seuls `width`/`height`, `status` et `qr_payloads` sont produits. Aucune représentation dérivée n'est créée dans ce POC (les données URI embarquent les bytes originaux : `derived_sha256 == sha256` par construction) ; toute dérive future devra hasher ses bytes dérivés indépendamment et conserver la référence parente.
...
- Mode désactivé : `MODEL_SUPPORTS_VISION=false` — la pile optionnelle (Pillow/zxing-cpp) n'est PAS importée par le chemin texte (preuve subprocess avec imports bloqués) ; `QR_DECODE_ENABLED=false` : aucun décodage QR n'est exécuté. `essential_visual_content=true` sans vision : aucun contenu visuel inventé ; le chemin `essential_visual_unread` existant (gate R3, policy REVIEW) reste inchangé.
