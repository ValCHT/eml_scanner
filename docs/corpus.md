...
**Labels de référence du POC exploratoire (amendement opérateur Gold-AI, 2026-09-20).** Les labels de référence de ce POC exploratoire sont produits par une annotation Silver ChatGPT suivie d'une annotation/adjuration indépendante GPT-6 Astra ; ce sont des **AI-adjudicated reference labels** et NON un ground truth analyste humain. L'artefact d'adjuration déclare véridiquement `final_status=ai_adjudicated`, `reviewer_ref=astra_gold_ai_v1`, `review_method=independent_dual_model_ai_adjudication`, `human_validated=false` ; ces faits restent vrais et ne sont jamais réécrits. Limitations méthodologiques admises et documentées : (1) la revue IA indépendante n'était pas parfaitement source-blind — des métadonnées résiduelles identifiant le corpus restaient observables dans certains messages bruts, accepté pour ce POC exploratoire ; (2) `menace` a zéro exemple de référence supporté et aucun exemple n'est fabriqué ; (3) `spear_phishing` n'a que 16 candidats Gold-AI ; (4) la Macro-F1 six classes est donc non concluante dès qu'une classe n'a aucun support ; (5) toute affirmation de production SOC exigerait une validation ultérieure par analystes humains.
...
`gold_dev.jsonl` contient uniquement les métadonnées, références locales et labels validés du GoldRecord de docs/contracts.md §2.8, dont raw_path et raw_sha256 ; aucun contenu email, HTML, header, MIME, pièce, image ou extrait dans les notes. Le contenu, privé compris, reste dans corpus/raw/** git-ignoré. Le loader résout le chemin, charge les bytes et vérifie leur SHA-256 avant analyse ; mismatch ou champ de contenu interdit invalide la validation G6. `gold_test.jsonl` a le même schema fermé (métadonnées/labels seulement) et constitue, pour ce POC exploratoire (amendement opérateur 2026-09-20), une **partition de validation interne** matérialisée dans le workspace de build avec `test_seal.json` (reçu agrégé pour la reproductibilité) ; ce n'est PAS un holdout indépendant, isolé ou aveuglé. Interdits maintenus : toute retouche de prompt, seuil, variante ou sélection de modèle fondée sur `gold_test` ; les décisions de développement utilisent `gold_dev` uniquement. Ni gold_dev ni gold_test ne sont indexés dans le RAG.
...
**Implémentation TICKET-15 (2026-09-21, gate G7-A).** Adaptateur `src/tools/rag.py` + CLI `scripts/manage_rag.py` :

- Base : une collection Chroma **embarquée** unique `public_rag_reference`, `PersistentClient` sous `RAG_DIR` (`corpus/rag/chroma/`, git-ignoré), espace **cosine**. Aucun service vectoriel distant, aucune autre base, pas de GraphRAG, pas de réécriture de requête par LLM.
- Embedding gelé : ONNX `all-MiniLM-L6-v2` fourni par chromadb (`ONNXMiniLM_L6_V2`, archive épinglée avec vérification SHA-256 par chromadb) ; l'empreinte SHA-256 réelle de `model.onnx` et l'identifiant `all-MiniLM-L6-v2@sha256:<16>` sont écrits dans les métadonnées de la collection et **vérifiés à chaque ouverture** : un espace d'embedding différent est refusé (clear + rebuild), jamais mélangé. Seul `text_excerpt` est vectorisé — label, rationale et identifiants n'entrent jamais dans le texte d'embedding.
- Refus structurels (`RagSourceCase`, schéma fermé) : `is_public=true` et `split=rag_reference` obligatoires, label de taxonomie, référence de validation non vide, URL publique, SHA-256 de record, extrait ≤ `max_case_chars` (1200). Un cas privé, gold (`split=dev|test`) ou non confirmé est refusé avant toute écriture ; `manage_rag.py add` charge en plus les familles et les hashes des partitions Gold (métadonnées uniquement, usage exclusif de contrôle d'isolation) et refuse toute intersection famille/hash — un cas par famille dans l'index, `max_cases=150`.
- Recherche (`search(query, exclusions, k=3)`) : tous les candidats demandés à Chroma puis filtrage local — `distance ≤ 0.40`, exclusions du message courant (`family_group`, `duplicate_group`, `campaign_id`), **au plus un voisin par family_group**, ordre déterministe `(distance, case_id)`, jamais de voisin forcé. Résultat = `RagCase` avec distance et empreinte d'embedding ; contexte d'inférence uniquement (INFERENCE), jamais ajouté au registre d'observables courant.
- Mode désactivé (baseline gelée `RAG_ENABLED=false`) : ni chromadb ni les poids ONNX ne sont importés/chargés ; `node_rag_lookup` reste un no-op vide. L'adaptateur n'est construit que si `tools.rag.enabled` ET `RAG_ENABLED` sont vrais, et son stockage reste paresseux jusqu'au premier usage.
...
**Normalisation d'empreinte (comparaison uniquement, §7.4).** La baseline applique NFKC + casefold + réduction des espaces (les répétitions de séparateurs sont absorbées par cette réduction). La substitution de tokens de tracking/URL que §7.4 réserve au fingerprint n'est **pas** appliquée dans cette baseline — choix conservateur assumé pour ne pas fusionner deux URLs distinctes dans une même famille ; aucune transformation non implémentée n'est revendiquée.

**Relecture raw de dedupe (fail-closed).** Le chemin `dedupe` qui redérive le texte d'empreinte depuis les octets bruts applique les mêmes règles que l'ingestion : noms de membres non sûrs (`..`, absolus) et membres au-delà de `MAX_MEMBER_BYTES` refusés sans être lus, borne agrégée `MAX_ARCHIVE_UNCOMPRESSED_BYTES` (512 MiB) par archive dépassée = erreur explicite. Avant tout fingerprint, les octets résolus doivent être non vides et `sha256(data) == raw_sha256` du manifest ; mismatch ou octets illisibles = échec explicite (`CorpusRawError`), le manifest n'est pas réécrit. Les lignes refusées à l'ingestion (`raw_sha256` vide) sont comptées séparément, jamais fingerprintées silencieusement.
