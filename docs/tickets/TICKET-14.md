...
`check_gate.py G6 --record` est rejouable sans nettoyage manuel : avant d'exécuter ses commandes, il déplace (jamais supprime) toute sortie smoke existante sous `runs/eval/_archive/<horodatage UTC>_<nom>` et consigne le déplacement dans le reçu (`archived_pre_run`).
