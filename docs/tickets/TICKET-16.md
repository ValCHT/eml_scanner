...
Capacité vision bornée — optionnelle maintenant, uniquement si le proxy vision est disponible :
...
Évaluation live appariée puis fermeture G7-B — à n'exécuter que sur décision/synchronisation opérateur :
...
**Implémentation (maintenant)** : Exit 0 ; tests déterministes verts ; statut `IMPLEMENTED_WAITING_FOR_LIVE_VISION_SMOKE` (smoke visuel live non exécuté) ou `IMPLEMENTED_WAITING_FOR_G6_SYNC` (smoke visuel live passé) ; aucune évaluation live appariée, aucun reçu de gate.

**Fermeture différée (plus tard)** : Exit 0 ; images effectivement transmises et payloads réellement décodées ; aucun scan local ni formulaire soumis ; limites respectées ; métriques ou INCONCLUSIVE si le subset est insuffisant.
...
Ticket PASS signifie : implémentation complète et validée dans la phase parallèle, statut `IMPLEMENTED_WAITING_FOR_LIVE_VISION_SMOKE` ou `IMPLEMENTED_WAITING_FOR_G6_SYNC` — la capacité vision sur le vrai proxy, l'utilité mesurée et G7-B sont jugées lors de la fermeture différée. Baseline toujours fonctionnelle ; test fermé.
...
Extension facultative vision/QR ; réponses réelles, comparaison et décision G7-B produites lors de la fermeture différée.

## CODEX EXECUTION PROMPT

Exécute uniquement TICKET-16 — Vision et QR sans second pipeline, gate G7-B, en **phase d'implémentation parallèle**. Le présent fichier complet est ton prompt autonome. Lis AGENTS.md s'il existe, puis docs/architecture.md, docs/contracts.md, docs/decisions.md, docs/gates.md et les documents de domaine cités ici. Vérifie les préconditions ci-dessus, implémente exactement le périmètre dans FILES ALLOWED, lance uniquement les commandes de la section **IMPLEMENTATION VALIDATION** et arrête-toi au statut `IMPLEMENTED_WAITING_FOR_LIVE_VISION_SMOKE` (si le smoke visuel live n'a pas pu être exécuté) ou `IMPLEMENTED_WAITING_FOR_G6_SYNC` (s'il a passé) : c'est un succès de la phase parallèle, pas un BLOCKED/FAIL. Ne lance PAS la fermeture expérimentale différée (évaluation live appariée et `check_gate.py G7-B --record`) sans le point de synchronisation/G6 choisi par l'opérateur. Si un prérequis est absent, rends BLOCKED avec le fichier/capacité manquant ; ne le remplace pas par une hypothèse, un skipped ou une simulation. Corrige les échecs sans abaisser les tests. Termine par les fichiers changés, commandes et résultats effectifs, artefacts, statut du ticket (`IMPLEMENTED_WAITING_FOR_LIVE_VISION_SMOKE` ou `IMPLEMENTED_WAITING_FOR_G6_SYNC`) et statut de la gate (G7-B non fermée, différée). Ne commence aucun autre ticket. Aucun accès à un échange antérieur avec l'utilisateur n'est nécessaire.
