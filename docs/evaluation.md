...
**Implémentation T16 (2026-09-21).** `--variant vision --paired-with <run-dir>` est implémenté. Ordre d'exécution en live : vérification du lock → chargement **lecture seule** du run apparié (mêmes contrôles que la variante RAG) → validation Gold complète des 83 records → sélection smoke déterministe → identité exacte des `sample_id` + label + `raw_sha256` entre les deux séries → préparation de la capacité Vision **en mémoire** (`tools.vision.enabled=true` + `MODEL_SUPPORTS_VISION=true` effectifs : `configs/tools.yaml` et son hash de lock restent intacts ; le QR reste piloté séparément par `QR_DECODE_ENABLED` et n'est jamais activé ni requis implicitement — Pillow seul est exigé sans QR, zxing-cpp en plus si QR activé, sinon BLOCKED avant tout appel) → run live → artefact apparié. Le manifest archive `vision_ablation.capability` (overrides explicites, `qr_decode_enabled_effective`, `frozen_tools_yaml_unchanged`, dépendances), `vision_ablation.limits` (4 images / 4 MiB par image / 8 MiB au total / 16 M pixels) et le run apparié. Chaque ligne d'évaluation archive les `visual_cases` réels du rapport (`status` de chaque image : `metadata_only`/`supplied_to_model`/`unsupported`/`over_limit`/`unavailable`, dimensions, payloads QR exactes) et les compteurs §2.6.1 `visual_count_sent_internal`/`visual_count_sent_final`. `paired_baseline.json` contient l'identité de sélection, les dénominateurs, l'usage vision (`vision_usage` : échantillons avec pixels préparés, blocs réellement transmis d'après les audits archivés, payloads QR) et la comparaison appariée `paired_variant_block` — **publiée uniquement si** des pixels ont été préparés (`supplied_to_model`) ET que les audits archivés prouvent leur transmission ; sinon l'artefact est explicitement INCONCLUSIVE (`delta_published=false`, `comparison=null`, aucun delta inventé), et des pixels préparés absents des audits sont un FAIL (incohérence de câblage/audit, jamais un delta silencieux). L'attachement des pixels aux appels du graphe est câblé par T19C ; le premier benchmark complet texte/texte+QR/texte+QR+vision est T19E. Tests offline : `tests/test_evaluation.py` (refus sans `--paired-with`, dépendances manquantes = BLOCKED, zxing requis seulement si QR activé, happy path avec baseline vérifié inchangé bit à bit, cas INCONCLUSIVE sans pixels préparés, FAIL si pixels préparés non transmis) et `tests/test_vision.py` (indépendance vision/QR, `derived_sha256` réellement calculé et association erronée refusée, schéma du smoke réel).
