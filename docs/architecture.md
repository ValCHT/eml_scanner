...
**Décision runtime contraignante du 19/09/2026, précisée par l'opérateur le 20/09/2026 :** le POC courant utilise l'API OpenAI-compatible AkashML (`https://api.akashml.com/v1/chat/completions`). `Qwen/Qwen3.8-27B` reste l'unique modèle des runs POC officiels (évidences de gate, mesures baseline G2/G5/G6, INTERNAL MEDIUM et FINAL XHIGH). `openai/gpt-oss-20b` est réservé aux tests techniques les moins chers (connectivité/transport/smoke). `Qwen3.6-35B-A3B` est autorisé par l'opérateur pour les seules boucles de développement non officielles : tests live fonctionnels/de débogage nécessitant réellement un LLM et pré-validation avant un run officiel. Ses sorties ne sont jamais présentées comme des preuves de gate Qwen3.8 ni comme des mesures baseline ; aucun ID fournisseur Akash n'est revendiqué pour ce modèle, l'ID exact doit provenir de la liste de modèles de l'endpoint configuré. La précédente exécution G2 Genspark + `claude-haiku-4-5` reste un fait historique, pas une preuve runtime courante. Orange LiteLLM reste la cible de démonstration future via les mêmes variables génériques `LITELLM_*`.
...
| Modèles et rôles | `openai/gpt-oss-20b` : tests techniques/smoke les moins chers uniquement. `Qwen/Qwen3.8-27B` : INTERNAL/FINAL et toutes les évidences et mesures officielles (G2/G5/G6, baseline, INTERNAL MEDIUM, FINAL XHIGH). `Qwen3.6-35B-A3B` : développement non officiel uniquement (boucles bon marché, tests live fonctionnels/débogage, pré-validation avant run officiel) ; jamais une preuve de gate ni une mesure baseline, ID fournisseur à reprendre de l'endpoint configuré. Haiku : fait historique, pas un chemin runtime. |
